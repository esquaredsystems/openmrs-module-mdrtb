			...
			<td width="25%" valign="top" class="patientTable">
				<table>
					<tr style="border-bottom:2px solid black; height:25px;"><td colspan="2" style="padding-left:10px; background-color:#C0C0C0; font-weight:bold;">
						<mdrtb:message code="mdrtb.choosePatientsToDisplay" text="Choose patients to display"/>:
						<input type="submit" value="<mdrtb:message code="mdrtb.listPatients" text="List"/>"/>
					</td></tr>
					<!-- <tr><td colspan="2">
						<br/>
						<table>
							<tr>
								<th><mdrtb:message code="general.name"/>&nbsp;</th>
								<td><input type="text" name="name" value="${name}"/></td>
							</tr>
							<tr>
								<th><mdrtb:message code="general.id"/>&nbsp;</th>
								<td><input type="text" name="identifier" value="${identifier}"/></td>
							</tr>
						</table>
						<br/>
					</td></tr> -->
					<tr><th colspan="2"><mdrtb:message code="mdrtb.oblast" text="Oblast"/>
							<select name="oblast">
								<option value=""></option>
					...
					<tr><th colspan="2"><mdrtb:message code="mdrtb.enrolledLocation" text="Enrolled Location"/>
					...
					<tr><th><mdrtb:message code="mdrtb.minimumAgeAtTxStart" text="Minimum Age at<br>Treatment Start"/>&nbsp;</th><td><input type="text" name="minage" value="${minage}" /></td></tr>
					<tr><th><mdrtb:message code="mdrtb.maximumAgeAtTxStart" text="Maximum Age at<br>Treatment Start"/>&nbsp;</th><td><input type="text" name="maxage" value="${maxage}" /></td></tr>
					
				
						</table> -->
						<br/>
					<tr>
					<th colspan="2"><mdrtb:message code="mdrtb.gender" text="Gender"/>&nbsp;<select name="gender" >
					<option value=""></option>
					<option value="M"><mdrtb:message code="mdrtb.male" text="Male"/></option>
					<option value="F"><mdrtb:message code="mdrtb.female" text="Female"/></option>
					</select>
					</th>
					</tr>
					<tr><th colspan="2"><br/><mdrtb:message code="mdrtb.enrollment"/></th></tr>
					<tr>
						<td><mdrtb:message code="mdrtb.year"/></td>
						<td><input type="text" name="year" value="${year}"/>
					</tr>
					<tr>
						<td><mdrtb:message code="mdrtb.quarter"/></td>
						<td><input type="quarter" name="quarter" value="${quarter}"/>
					</tr>
					<tr>
						<td><mdrtb:message code="mdrtb.or"/></td>
					</tr>
					<tr>
						<td><mdrtb:message code="mdrtb.month"/></td>
			...
			<td valign="top" width="100%" class="patientTable" style="padding-left:10px;">
				<table width="100%">
					<tr style="border-bottom:2px solid black;"><td colspan="2" style="background-color:#C0C0C0; font-weight:bold;">
						<mdrtb:message code="mdrtb.chooseColumnsToDisplay" text="Choose columns to display"/>: 
						<select name="displayMode">
							<option value="basic"<c:if test="${'basic' == param.displayMode}"> selected</c:if>><mdrtb:message code="mdrtb.basicDetails"/></option>
							<option value="mdrtbShortSummary"<c:if test="${'mdrtbShortSummary' == param.displayMode}"> selected</c:if>><mdrtb:message code="mdrtb.mdrtbShortSummary"/></option>
							<option value="mdrtbCustomList"<c:if test="${'mdrtbCustomList' == param.displayMode}"> selected</c:if>><mdrtb:message code="mdrtb.mdrtbCustomList"/></option>
							...
							<openmrs:extensionPoint pointId="org.openmrs.mdrtb.listPatientDisplayModes" type="html">
								<option value="${extension.key}"<c:if test="${extension.key == param.displayMode}"> selected</c:if>>
									<mdrtb:message code="${extension.label}"/>
								</option>
										...
										<tr class="patientRow patientRow${p.patientId}">
											<td class="patientTable" style="white-space:nowrap; width:20px;">
												...
												<openmrs:extensionPoint pointId="org.openmrs.mdrtb.listPatientDetailPortlets" type="html">
													<a href="javascript:void(0)" onClick="loadUrlIntoDetailsPopup('<mdrtb:message code="${extension.title}"/>', '${pageContext.request.contextPath}/module/mdrtb/viewPortlet.htm?patientId=${p.patientId}&moduleId=${extension.moduleId}&id=${extension.moduleId}&url=${extension.portletUrl}'); return false;">
														<img src="${pageContext.request.contextPath}${extension.imagePath}" title="<mdrtb:message code="${extension.title}"/>" border="0" align="top" />
													</a>
												</openmrs:extensionPoint>
											</td>
...
<div id="displayDetailsPopup"> 
	<div id="displayDetailsPopupLoading"><mdrtb:message code="general.loading"/></div>
