...
<script type="text/javascript">
	...
	function validateAndSubmit() {
		...
		if (generic == '') {
			$j('#errorDisplay').append("<li><mdrtb:message code='mdrtb.drugOrder.genericRequired' text='Please specify a Drug'/></li>");
		}
		if (startDate == '') {
			$j('#errorDisplay').append("<li><mdrtb:message code='mdrtb.drugOrder.startDateRequired' text='Please specify a Start Date'/></li>");
		}
		if (dose != '') {
	    ...
	    if (discontinuedDate != '') {
	    	if (discontinuedReason == '') {
	    		$j('#errorDisplay').append("<li><mdrtb:message code='mdrtb.drugOrder.discontinuedReasonRequired' text='Please specify a discontinued reason'/></li>");
	    	}
	    	// TODO: Check if this is after startDate and before autoExpireDate (if autoExpire not null)
	    }
	    if (discontinuedReason != '') {
	    	if (discontinuedDate == '') {
	    		$j('#errorDisplay').append("<li><mdrtb:message code='mdrtb.drugOrder.discontinuedDateRequired' text='Please specify a discontinued date'/></li>");
	    	}
	    }
...
<form id="drugOrderForm" action="saveDrugOrder.form">
	...
	<b class="boxHeader">
		<c:choose>
			<c:when test="${empty orderId}"><mdrtb:message code="mdrtb.new" text="New"/></c:when>
			<c:otherwise><mdrtb:message code="mdrtb.edit" text="Edit"/></c:otherwise>
		</c:choose>
		<mdrtb:message code="mdrtb.treatment.${type}"/> 
		<mdrtb:message code="mdrtb.drugOrder" text="Drug Order"/> 
	</b>
	<div class="box">
		...
		<table>
			...
			<tr>
				...
				<td class="cellStyle">
					<select id="drugSelector" name="drug">
						<option value=""><mdrtb:message code="mdrtb.unspecified" text="Unspecified"/></option>
			...
			<tr>
				...
				<td class="cellStyle">
					<c:choose>
						...
						<c:otherwise>
							<input type="text" size="5" name="perDay"/><mdrtb:message code="mdrtb.perday"/>
							<input type="text" size="5" name="perWeek"/><mdrtb:message code="mdrtb.perweek"/>
						</c:otherwise>
					</c:choose>
				</td>
			</tr>
			...
			<tr>
				<th class="headerStyle"><mdrtb:message code="mdrtb.discontinueddate" text="Discontinued Date"/></th>
				<td class="cellStyle">
			...
			<tr>
				<th class="headerStyle"><mdrtb:message code="mdrtb.reasonDiscontinued" text="Reason discontinued"/></th>
				<td class="cellStyle">
		...
		<input type="button" onclick="validateAndSubmit();" value="<mdrtb:message code="mdrtb.submit" text="Submit"/>"/>
