...
import org.openmrs.module.mdrtb.web.dto.SimpleTransferOutForm;
...
@Resource(name = RestConstants.VERSION_1 + "/mdrtb/transferout", supportedClass = SimpleTransferOutForm.class, supportedOpenmrsVersions = { "2.2.*,2.3.*,2.4.*,2.8.*" })
public class TransferOutFormResourceController extends DataDelegatingCrudResource<SimpleTransferOutForm> {
	...
	@Override
	public SimpleTransferOutForm newDelegate() {
		return new SimpleTransferOutForm();
	}
	...
	@PropertySetter("obs")
	public static void setObs(SimpleTransferOutForm instance, Set<Obs> obs) {
	...
	protected PageableResult doSearch(RequestContext context) {
		...
		if (patientUuid != null) {
			...
			for (Encounter encounter : encs) {
				if (encounter.getEncounterType().equals(MdrtbConstants.ET_TRANSFER_OUT)) {
					TransferOutForm transferOutForm = new TransferOutForm(encounter);
	...
	@Override
	public SimpleTransferOutForm getByUniqueId(String uuid) {
		Encounter encounter = Context.getEncounterService().getEncounterByUuid(uuid);
		SimpleTransferOutForm simpleTransferOutForm = new SimpleTransferOutForm(new TransferOutForm(encounter));
		return simpleTransferOutForm;
	}
	
	@Override
	public SimpleTransferOutForm save(SimpleTransferOutForm delegate) throws ResponseException {
		...
		TransferOutForm transferOutForm = new TransferOutForm(delegate.getEncounter());
		transferOutForm = formService.processTransferOutForm(transferOutForm);
		return new SimpleTransferOutForm(transferOutForm);
	}
	
	@Override
	protected void delete(SimpleTransferOutForm delegate, String reason, RequestContext context) throws ResponseException {
		Context.getEncounterService().voidEncounter(delegate.getEncounter(), reason);
	}
	
	@Override
	public void purge(SimpleTransferOutForm delegate, RequestContext context) throws ResponseException {
