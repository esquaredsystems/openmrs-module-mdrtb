...
public class RedirectSomeRolesFromOpenmrsHomepage extends Extension {
	 ...
	 * @should do nothing if none of the authenticated users roles are in the ROLES_TO_REDIRECT
	 *         global property
	 * @should redirect the user via javascript if one of the authenticated users roles is in the
	 *         ROLES_TO_REDIRECT global property
	 */
	@Override
	public String getOverrideContent(String str) {
		...
		// Replaced OpenmrsConstants.PRIV_VIEW_GLOBAL_PROPERTIES with PrivilegeConstants.GET_GLOBAL_PROPERTIES
		Context.addProxyPrivilege(PrivilegeConstants.GET_GLOBAL_PROPERTIES);
		try {
			String roleNamesToRedirect = Context.getAdministrationService().getGlobalProperty(
			    MdrtbConstants.ROLES_TO_REDIRECT_GLOBAL_PROPERTY);
