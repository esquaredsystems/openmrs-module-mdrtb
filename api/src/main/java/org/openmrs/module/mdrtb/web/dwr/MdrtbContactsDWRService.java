...
public class MdrtbContactsDWRService {
	...
	public boolean updateAddress(Integer personId, Integer addressId, String address1, String address2, String township,
	        String city, String district, String region, String phone) {
		boolean ret = true;
		try {
			...
			for (PersonAddress pa : contact.getAddresses()) {
				if (pa.getPersonAddressId().intValue() == addressId) {
					...
					if ((pa.getAddress1() == null && address1 != null)
					        || (pa.getAddress1() != null && !pa.getAddress1().equals(address1))) {
						pa.setAddress1(address1);
						saveTest = true;
					}
					if ((pa.getAddress2() == null && address2 != null)
					        || (pa.getAddress2() != null && !pa.getAddress2().equals(address2))) {
						pa.setAddress2(address2);
						saveTest = true;
					}
					if ((pa.getAddress4() == null && township != null)
					        || (pa.getAddress4() != null && !pa.getAddress4().equals(township))) {
						pa.setAddress4(township);
						saveTest = true;
					}
					if ((pa.getCityVillage() == null && city != null)
					        || (pa.getCityVillage() != null && !pa.getCityVillage().equals(city))) {
						pa.setCityVillage(city);
						saveTest = true;
					}
					if ((pa.getCountyDistrict() == null && district != null)
					        || (pa.getCountyDistrict() != null && !pa.getCountyDistrict().equals(district))) {
						pa.setCountyDistrict(district);
						saveTest = true;
					}
					if ((pa.getAddress6() == null && region != null)
					        || (pa.getAddress6() != null && !pa.getAddress6().equals(region))) {
						pa.setAddress6(region);
						saveTest = true;
					}
					if (saveTest)
			...
			List<Obs> oListPhone = os.getObservationsByPersonAndConcept(contact, Context.getService(MdrtbService.class)
			        .getConcept(MdrtbConcepts.TELEPHONE_NUMBER));
			...
			ret = false;
			log.info("updateAddress DWR service failed", ex);
		}
		
		return ret;
	}
	
	public boolean updateContact(Integer patientId, Integer personId, Integer relationshipId,
	        String newRelationshipTypeString, String givenName, String familyName, String popupContactIdVal,
	        Integer popupKnownMDRVal) {
		boolean ret = true;
		try {
			PersonService perS = Context.getPersonService();
			Person contact = perS.getPerson(personId);
			...
			if ((pn.getGivenName() == null && givenName != null)
			        || (pn.getGivenName() != null && !pn.getGivenName().equals(givenName))) {
				pn.setGivenName(givenName);
				saveTest = true;
			}
			if ((pn.getFamilyName() == null && familyName != null)
			        || (pn.getFamilyName() != null && !pn.getFamilyName().equals(familyName))) {
			...
			if (contactAttType != null && popupContactIdVal != null && !popupContactIdVal.equals("")) {
				...
				for (PersonAttribute patt : contact.getActiveAttributes()) {
					if (!patt.isVoided() && patt.getAttributeType().equals(contactAttType)
					        && !patt.getValue().equals(popupContactIdVal)) {
			...
			String relationshipKey = newRelationshipTypeString.substring(newRelationshipTypeString.length() - 1,
			    newRelationshipTypeString.length());
			newRelationshipTypeString = newRelationshipTypeString.substring(0, newRelationshipTypeString.length() - 1);
			boolean test = false;
			if (r.getRelationshipType().getRelationshipTypeId().intValue() != Integer.valueOf(newRelationshipTypeString)
			        .intValue()) {
			...
			if (popupKnownMDRVal != null) {
				...
				if (popupKnownMDRVal != null) {
					ObsService os = Context.getObsService();
					Concept cKnown = Context.getService(MdrtbService.class).getConcept(
					    MdrtbConcepts.CONTACT_KNOWN_OR_CURRENT_MDR_CASE);
	...
	public PersonAddress createNewAddress(Integer personId, String address1, String address2, String township, String city,
	        String district, String region, String phone) {
		...
		try {
			...
			log.error("DWR unable to create peronAddres", ex);
		}
		return pa;
	}
		...
		try {
			...
			for (Relationship r : ps.getRelationshipsByPerson(supporter)) {
				if (r.getPersonA().getPersonId().intValue() == supporter.getPersonId().intValue()
				        ...
				        && r.isVoided() == false
				        && r.getRelationshipType().getRelationshipTypeId().intValue() == rt.getRelationshipTypeId()
				                .intValue()) {
