...
public class PatientChartFactory {
	 ...
	 */
	private Record createChartRecord(List<Specimen> specimens, List<RecordComponent> stateChangeRecordComponents,
	        RegimenHistory regimenHistory, Calendar recordStartDate, Calendar recordEndDate) {
		// get the specimen components for this period
		List<RecordComponent> components = createSpecimenRecordComponents(specimens, regimenHistory, recordStartDate,
		    recordEndDate);
		...
		if (components.size() == 0) {
			...
			// regimen information)
			components.add(new SpecimenRecordComponent((recordStartDate != null ? new Date(recordStartDate.getTime()
			        .getTime() + 1) : null), regimens));
		}
