...
/**
 * Represents a series of drugs given at the same time, generally for the same reason. The startDate
 * and endDate of the Regimen represent the start dates of the whole Regimen: its individual
 * components may have different startDate and endDate values.
 */
public class Regimen {
	 ...
	 * @param entirePeriod if true, checks that the Regimen is active for the entire period.
	 *            otherwise, checks that it is active ever during the period.
	...
	/**
	 * @return the duration of the Regimen in days. If the start date is null or after the end date,
	 *         returns -1
	 */
	...
	public Set<Concept> getEndReasons() {
		...
		if (getEndDate() != null) {
			for (DrugOrder o : getDrugOrders()) {
				...
				if (getEndDate().equals(o.getEffectiveStopDate()) && o.getOrderReason() != null) {
					c.add(o.getOrderReason());
				}
			}
		}
		return c;
