...
public class RegimenHistory {
	...
	public void addDrugOrder(DrugOrder order) {
		...
		RegimenChange startChange = getRegimenChanges().get(order.getEffectiveStartDate());
		if (startChange == null) {
			startChange = new RegimenChange(order.getEffectiveStartDate());
			getRegimenChanges().put(order.getEffectiveStartDate(), startChange);
		}
	...
	/**
	 * @return the List of active DrugOrders at the start of the passed Date (i.e. not including
	 *         changes effective that day)
	 */
