...
public class MdrtbAfterTreatmentStartedCohortDefinitionEvaluator extends MdrtbTreatmentStartedCohortDefinitionEvaluator {
	...
	public EvaluatedCohort evaluate(CohortDefinition cohortDefinition, EvaluationContext context) {
		 ...
		 * c = MdrtbQueryService.getPatientsResistantToAnyDrugs(context, sd, ed, inh); // Must be
		 * resistant to RIF Concept rif =
		 * Context.getService(MdrtbService.class).getConcept(MdrtbConcepts.RIFAMPICIN); c =
		 * Cohort.intersect(c, MdrtbQueryService.getPatientsResistantToAnyDrugs(context, sd, ed,
		 * rif));
		 */
		...
		for (CohortMembership membership : memberships) {
			...
			if (programs != null && programs.size() != 0) {
				...
				Concept inh = Context.getService(MdrtbService.class).getConcept(MdrtbConcepts.ISONIAZID);
				Cohort c = MdrtbQueryService.getPatientsResistantToAnyDrugs(context, startDateCollected, endDateCollected,
				    inh);
				
				// Must be resistant to RIF
				Concept rif = Context.getService(MdrtbService.class).getConcept(MdrtbConcepts.RIFAMPICIN);
				c = Cohort.intersect(c,
				    MdrtbQueryService.getPatientsResistantToAnyDrugs(context, startDateCollected, endDateCollected, rif));
