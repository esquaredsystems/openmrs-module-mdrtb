...
public class SpecimenReportingTools {
	...
	/**
	 * Returns all specimens collected within the start and end date parameters that have no smear,
	 * bacteriology, or dst results associated with them
	 */
	public static Map<Patient, List<Specimen>> getSpecimensWithNoResults(Date startDateCollected, Date endDateCollected) {
		...
		String sql = "select patient_id, encounter_id from encounter where encounter_type='"
		        + specimenEncounterType.getId()
		        + "' and voided='0' and encounter.encounter_id "
		        + (startDateCollected != null ? "and encounter.encounter_datetime >= '"
		                + sqlFormat.format(startDateCollected) + "' " : "")
		        ...
		        + "and obs.voided='0' and value_coded is not null and " + "(obs.concept_id='" + smearResult.getId()
		        + "' or obs.concept_id='" + cultureResult.getId() + "' " + "or obs.concept_id='" + resistant.getId()
		        + "' or obs.concept_id='" + intermediate.getId() + "' or obs.concept_id='" + suceptible.getId() + "') )";
	...
	public static Map<Patient, List<Specimen>> getSpecimensWithPositiveCultureButNoDstResults(Date startDateCollected,
	        Date endDateCollected, Integer daysSinceCulture, Integer[] labIds) {
		...
		Concept intermediate = Context.getService(MdrtbService.class).getConcept(MdrtbConcepts.INTERMEDIATE_TO_TB_DRUG);
		Concept suceptible = Context.getService(MdrtbService.class).getConcept(MdrtbConcepts.SUSCEPTIBLE_TO_TB_DRUG);
		Concept waitingForTestResults = Context.getService(MdrtbService.class).getConcept(
		    MdrtbConcepts.WAITING_FOR_TEST_RESULTS);
		...
		String sql = "select distinct encounter.patient_id, encounter.encounter_id from encounter, obs where encounter.encounter_id = obs.encounter_id "
		        ...
		        + specimenEncounterType.getId()
		        + "' and encounter.voided='0' and obs.voided='0' "
		        + "and value_coded is not null and value_coded != '"
		        + waitingForTestResults.getId()
		        + "' "
		...
		triage: for (List<Object> result : results) {
	...
	public static Map<Patient, List<Specimen>> getSpecimensWithPositiveSmearButNoCultureResults(Date startDateCollected,
	        Date endDateCollected, Integer daysSinceSmear, Integer[] labIds) {
	...
	/**
	 * Returns all specimens collected within the start and end date parameters that have a test
	 * marked as "contaminated"
	 */
	public static Map<Patient, List<Specimen>> getSpecimensWithContaminatedTests(Date startDateCollected,
	        Date endDateCollected, Integer[] labIds) {
		...
		String sql = "select distinct encounter.patient_id, encounter.encounter_id from encounter, obs where encounter.encounter_id = obs.encounter_id "
		        ...
		        + contaminated.getId()
		        + ","
		        + unsatisfactory.getId() + ")) " + "or concept_id = '" + dstContaminated.getId() + "');";
	...
	/**
	 * Takes a SQL query result and converts it into the format we wish to return... a map with
	 * patients as the key and the list of specimens associated with that patient as the value
	 */
